...
def calculate_normalized_rmse(predictions, observations, variable_names):
    """
    Calculate mean normalized RMSE across multiple variables
...
def run_calibration_de(model_dir, forcing_file, obs_file, bounds_file,
    ...
    for run_idx in range(num_calibration):
        ...
        def objective(calibrate_param_values):
            ...
            # Calculate error
            error, var_errors = calculate_normalized_rmse(predictions, obs_data, data_dict['target_var_names'])

            if n_evals[0] % 20 == 0:
                print(f"  Eval {n_evals[0]}: NRMSE = {error:.6f}")
